...
use crate::services::market_data_provider::upstox::UpstoxClient;
...
const SYMBOL_PRICE_PREFIX: &str = "market_data:upstox:symbol:";

/// Key for tracking accessed symbols
const ACCESSED_SYMBOLS_KEY: &str = "market_data:upstox:accessed_symbols";

/// Service for managing market data using Upstox API
#[derive(Clone)]
pub struct UpstoxMarketDataService {
    redis: RedisManager,
    provider: Arc<UpstoxClient>,
    cache_duration: i64,
    stale_threshold: i64,
    update_lock: Arc<Mutex<()>>,
}

impl UpstoxMarketDataService {
    ...
    pub fn new() -> Self {
        ...
        let api_key = env::var("UPSTOX_API_KEY")
        ...
        // Create the Upstox provider
        let upstox_client = UpstoxClient::new(api_key);
        let provider = Arc::new(upstox_client);
...
impl UpstoxMarketDataService {
    ...
    pub async fn start_background_updater(service: Arc<Self>) {
        ...
        tracing::info!("Starting Upstox market data background updater with interval of {} seconds", update_interval);

        // Spawn the background task with immediate first update
        tokio::spawn(async move {
            tracing::info!("Starting initial Upstox market data update");
            ...
            match tokio::time::timeout(
                ...
                Ok(Ok(_)) => tracing::info!("Initial Upstox market data update completed successfully"),
                Ok(Err(e)) => tracing::error!("Initial Upstox market data update failed: {}", e),
                Err(_) => tracing::error!("Initial Upstox market data update timed out after 30 seconds"),
            }
            ...
            loop {
                ...
                tracing::debug!("Running scheduled Upstox market data update");
                match tokio::time::timeout(
                    StdDuration::from_secs(30),
                    service.update_all_cached_data()
                ).await {
                    Ok(Ok(_)) => tracing::debug!("Scheduled Upstox market data update completed"),
                    Ok(Err(e)) => tracing::error!("Scheduled Upstox market data update failed: {}", e),
                    Err(_) => tracing::error!("Scheduled Upstox market data update timed out after 30 seconds"),
                }
            }
        });
    }
...
#[async_trait]
impl MarketDataProvider for UpstoxMarketDataService {
    ...
    async fn get_symbol_prices(&self, symbols: &[String]) -> Result<BatchPriceResponse, ApiError> {
        ...
        if !symbols_to_fetch.is_empty() {
            tracing::debug!("Fetching {} symbols from Upstox provider", symbols_to_fetch.len());
            ...
            if fresh_prices.is_empty() && !symbols_to_fetch.is_empty() {
                tracing::warn!("No data returned from Upstox for symbols: {:?}", symbols_to_fetch);

                // Return a specific error if no symbols were found
                if cached_prices.is_empty() {
                    return Err(ApiError::NotFound(format!(
                        "No data available for the requested symbols. They may not be supported by Upstox."
                    )));
                }
            }
    ...
    async fn remove_stale_symbols(&self) -> Result<(), ApiError> {
        ...
        tracing::info!("Removing {} stale symbols from Upstox cache", stale_symbols.len());
    ...
    async fn update_all_cached_data(&self) -> Result<(), ApiError> {
        ...
        tracing::info!("Updating {} Upstox symbols", symbols.len());
        ...
        for chunk in symbols.chunks(batch_size) {
            ...
            let future = async move {
                match provider.fetch_market_data(&chunk_symbols).await {
                    ...
                    Err(e) => {
                        tracing::error!("Failed to update symbols batch: {}", e);
                        Err(e)
                    }
                }
            };
        ...
        tracing::info!("Updated {} Upstox symbols successfully, {} batch errors", success_count, error_count);
    ...
    async fn subscribe_to_symbols(&self, symbols: &[String]) -> Result<(), ApiError> {
        // Upstox doesn't support real-time updates through this API
        // This is a placeholder implementation
        tracing::warn!("Real-time updates not supported for Upstox: {:?}", symbols);
        Ok(())
    }
